...
public:
    void join();
    // creates new crawler
    static Crawler from(std::string const& url, size_t depth, size_t downloader_threads, size_t parser_threads,
                        std::string const& output_file_name);

private:
    // default initializer with no given jobs
    Crawler(std::size_t depth, std::string const& root_url, size_t downloader_threads, size_t parser_threads,
            std::string const& output_file_name);
