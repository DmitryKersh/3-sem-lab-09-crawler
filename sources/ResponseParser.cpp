...
std::ostream& operator<< (std::ostream& os, ParsingResult const& result){
  os << "Result:\n\tImages:\n";
  for (auto const& url : result.image_urls){
    os << "\t\t" << url << std::endl;
  }

  os << "\tChild URLs:\n";
  for (auto const& url : result.child_urls){
    os << "\t\t" << url << std::endl;
  }
...
std::set<std::string> recursive_search(GumboNode const* const node, GumboTag const tag, char const* const name){
  ...
  for (::std::size_t i = 0; i < length; ++i) {
    auto const child_refs = recursive_search(
        static_cast<GumboNode const*>(children->data[i]), tag, name);
