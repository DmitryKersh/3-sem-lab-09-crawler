...
Crawler::Crawler(std::size_t depth, const std::string &root_url, size_t downloader_threads, size_t parser_threads,
...
Crawler Crawler::from(std::string const &url, ::std::size_t depth, ::std::size_t downloader_threads,
    ...
    return Crawler{depth, url, downloader_threads == 0 ? ::std::thread::hardware_concurrency() : downloader_threads,
                   parser_threads == 0 ? ::std::thread::hardware_concurrency() : parser_threads, output_file_name};
}
...
void Crawler::publish_parser_job(ParserJob &&job) {
    ...
    asio::post(download_threadpool_, [this, job] {
        ...
        // writing image urls
        for (auto image_url : result.image_urls) publish_file_writer_job(std::move(image_url));

        if (parse_children) {
            for (auto child_url : result.child_urls) publish_downloader_job({std::move(child_url), depth_ + 1});
        }
